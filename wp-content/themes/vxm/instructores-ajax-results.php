	...
	<?php while ( have_posts() ) : the_post();
		...
		<li class="section wow fadeIn" data-wow-delay="<?php echo $counter*2; ?>0ms">
			<h5 class="no-margin pdt">
				<?php the_title(); ?>
				<span><a class="box-picture post-link" href="<?php the_permalink(); ?>" data-mfp-src="<?php the_permalink(); ?>" rel="<?php the_ID(); ?>" data-effect="mfp-modal"><small><i class="ti-pencil"></i> Edi</small></a></span>
			</h5>
			
			<?php $noiveo_or_id = !empty($noiveo) ? $noiveo : 'ID'.str_pad($cfuserid, 5, '0', STR_PAD_LEFT); ?>
			<small class="blue-grey-text"><?php echo $noiveo_or_id; ?> — <span class="badge-status status-<?php echo $statusraw; ?>"><?php echo $status; ?></span><?php/* — Fecha de Alta:  echo $alta; */?></small>
