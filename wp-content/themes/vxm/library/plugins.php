...
class Featured_Post
{
    ...
    function add_views_link($views) {
        ...
        $views['featured'] = "<a class=\"" . $class . "\" id=\"featured-post-filter\" href=\"edit.php?&post_status=featured&post_type={$post_type}\">Recomendados <span class=\"count\">({$count})</span></a>";
        return $views;
    }
    ...
    function manage_posts_columns($columns) {
        ...
        if (current_user_can('edit_posts', $user_id)) {
            $columns['featured'] = __('Recomendado');
        }
        return $columns;
    }
		...
		jQuery(document).ready(function($){
			$(\'.featured-post-toggle\').on("click",function(e){
				...
				$.ajax({url:ajaxurl,data:data,type:\'post\',
					...
					_el.removeClass(\'icon-like2\').removeClass(\'icon-like\');
					$("#featured-post-filter span.count").text("("+data.total_featured+")");
					if(data.new_status=="yes"){
						_el.addClass(\'icon-like2\');
					}else{
						_el.addClass(\'icon-like\');
					}
					}
